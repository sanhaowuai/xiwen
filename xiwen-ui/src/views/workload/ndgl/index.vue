...
<script>
  import { listNdgl, getNdgl, delNdgl, addNdgl, updateNdgl } from "@/api/workload/ndgl";
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listNdgl(this.queryParams).then(response => {
          this.ndglList = response.rows;
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              updateNdgl(this.form).then(response => {
                this.$modal.msgSuccess("修改成功");
                this.open = false;
                this.getList();
              });
            } else {
              addNdgl(this.form).then(response => {
      ...
      /** 导出按钮操作 */
      handleExport() {
        this.download('workload/ndgl/export', {
          ...this.queryParams
        }, `ndgl_${new Date().getTime()}.xlsx`)
      }
    }
  };
</script>
