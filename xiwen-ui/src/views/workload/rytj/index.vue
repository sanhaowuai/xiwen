<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="68p0">
      ...
      <el-form-item label="人员" prop="yhid">
        <el-select v-model="queryParams.yhid" filterable placeholder="请选择人员" style="width: 180px">
          <el-option
            v-for="ryxx in ryxxList"
            :key="ryxx.yhid"
            :label="ryxx.yhxm"
            :value="ryxx.yhid"
          />
...
<script>
  import { getMrbmxx,getGrzfList,getRyxxList } from "@/api/workload/wftjgl";
  ...
  export default {
    ...
    methods: {
      ...
      getYzDataList(){
        getGrzfList({nd:this.queryParams.nd}).then(res => {
      ...
      getMrbmxxv(){
        getMrbmxx().then(res => {
          this.queryParams.deptid = res.data.deptid
            getRyxxList().then(res => {
              this.queryParams.yhid = res.data.yhid;
              this.ryxxList = res.data.ryxxList;
              this.getBmzfListv();
            })
        })
      },
      getBmzfListv(){
        getGrzfList({nd:this.queryParams.nd,yhid:this.queryParams.yhid}).then(res => {
